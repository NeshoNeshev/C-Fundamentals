    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (input != "3:1")
            {
                ...
                {
                    int index = int.Parse(commandElements[1]); // 0
                    int partitions = int.Parse(commandElements[2]); // 3

                    if (index >= 0 && index <= data.Count - 1)
                    {
                        string word = data[index]; // abcd
                        List<string> dividedWord = new List<string>();
                        int stringLengthToAdd = word.Length / partitions; // 1
